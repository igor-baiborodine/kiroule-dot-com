...
This order is dictated by the propagation logic I built into the CI. When a change is pushed to a module like `policy-service-api`, the `legacy-release-api.yaml` workflow publishes the new JAR and identifies downstream services that require a version bump. However, I deliberately blocked automatic version bumping between dependent APIs—such as `documents-service-api` referencing `policy-service-api`—to prevent uncontrolled cascading releases within the contract layer. This choice forces a manual, explicit update of the version in the dependent API's `pom.xml` before pushing, ensuring that changes to core domain objects are intentionally reviewed and acknowledged by downstream contract owners.
